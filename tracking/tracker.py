...
class LandingPadTracker:
    ...
    def initialize(self, frame, corners) -> bool:
        ...
        if points is None:
            return False
        # The four observed paper corners are explicit LK anchors. Interior
        # features validate their motion rather than extrapolating all corners
        # from a small cluster on one X stroke. Do not count duplicates twice.
        distance = np.linalg.norm(points.reshape(-1, 1, 2)-work_corners[None, :, :], axis=2)
        points = np.concatenate((work_corners[:, None, :], points[distance.min(axis=1) >= 5]))[:self.max_points]
    ...
    def update(self, frame) -> TrackingOutput:
        ...
        if not keep[:4].all():
            return self._lost('corner_anchor_lost')
        ...
        if not keep[:4].all():
            return self._lost('corner_anchor_inconsistent')
        ...
        # Prefer the directly observed corner motion when the RANSAC model
        # extrapolates away from those anchors. Independent interior matches
        # must still support this alternative homography.
        try:
            projected_anchors = transform_corners(self.reference_corners, H)
        except ValueError:
            return self._lost('invalid_projection')
        ...
        if (self.age+1) % 10 == 0:
            details = {}
            valid_x, quality = detect_x(rectify_pad(frame, new_corners*self.scale_xy, (280, 396)),
                                        diagnostics=details)
            self.last_x_check = dict(valid=valid_x, quality=quality,
                                     candidate_corners=(new_corners*self.scale_xy).tolist(),
                                     diagnostics=details)
            if not valid_x:
        ...
        if not np.array_equal(selected[:4], np.arange(4)):
            return self._lost('corner_anchor_outside')
        ...
        return TrackingOutput(True, self.corners.copy(), count, ratio, mean_error, 'tracked', self.last_x_check)
