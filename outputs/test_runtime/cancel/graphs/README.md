...
מקור הנתונים: `outputs\test_runtime\cancel/metrics.csv`, עם סף הקבלה מתוך `results.json`.
...
חציון זמן העיבוד: 15.71 מילישניות;
חציון ההשהיה: 25.04 מילישניות.
...
התקבלו 3 אומדנים תקינים מתוך 4 פריימים שעובדו.
...
```powershell
python -m metrics.export_report_figures --run outputs\test_runtime\cancel
```
