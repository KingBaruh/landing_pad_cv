...
מקור הנתונים: `outputs\test_runtime\complete/metrics.csv`, עם סף הקבלה מתוך `results.json`.
...
חציון זמן העיבוד: 16.04 מילישניות;
חציון ההשהיה: 23.24 מילישניות.
...
הגרף מציג Reprojection RMS ואת סף קבלת ה־Pose: 5.0000 פיקסלים בתמונת
העבודה. בריצה זו רוחב המקור הוא 1280 ורוחב תמונת העבודה הוא
...
התקבלו 29 אומדנים תקינים מתוך 44 פריימים שעובדו.
...
```powershell
python -m metrics.export_report_figures --run outputs\test_runtime\complete
```
